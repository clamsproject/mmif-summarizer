Code to create a summary of a MMIF file, only keeping those annotations that are useful metadata (which can reduce the size of the file by one or two orders of magnitude), and making some implicit relations between annotations and anchors in the source explicit.
...
Don't believe what it says in the header on the PyPI page, it will give errors because the clams-python dependency is not in TestPyPI. Instead do this:
...
This package comes with a command line script to create a JSON summary of a MMIF file and to create a mini website from the summary. 
