...
Some of the code in graph.py should go here.
...
def visualize_summary(fname: str):
    """Visualize the summary via a set of graphs."""
    summary = json.load(open(fname))
    print(summary.keys())
    _visualize_views(summary.get('views'), fname + '.views')
    _visualize_timeframes(summary, fname + '.tfs')
    _visualize_tags(summary.get('tags'), fname + '.tags')
...
def _visualize_tags(tags: list, fname: str):
    ...
    for tag in tags:
        ...
        dot.node(tag_name + 'bb', label='3000', color='darkgreen', shape='box')
        dot.edge(tag_text, tag_name + 'bb')
    dot.render(fname, format=FORMAT)


def visualize_mmif(fname: str):
    ...
    for view in mmif.views:
        for anno in view.annotations:
            ...
            if anno.at_type.shortname == 'Alignment':
                ...
                dot.node(identifier, shape=shape, color=color, label=get_label(anno))
    ...
    dot.render(fname, format=FORMAT, view=True)
...
def get_label(anno):
    ...
    if at_type == 'TimeFrame':
        ...
        return f'{identifier} {start} {ftype}' if ftype != 'None' else f'{identifier} {start}'
    elif at_type == 'Token':
        return f'{identifier}\n{anno.properties.get("text")}'
    elif at_type == 'NamedEntity':
        return f'{identifier} [{anno.properties.get("text")}]'
        ...
        return f'{identifier} {str(anno.properties.get("timePoint"))}'
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == '-s':
        visualize_summary(sys.argv[2])
    elif sys.argv[1] == '-m':
        visualize_mmif(sys.argv[2])
